...
class spectrum:
    ...
    def __init__(self,name,TARGET_THICKNESS,THICKNESS_UNCERTAINTY,OMEGA,THETA,S1,S3,CURRENT_OFFSET,CURRENT_GRADIENT,RUN_TIME,FS,ZBEAM, ZTARGET,ATARGET,CH1,e_Ch1,Rest,e_R,ISOTOPIC_PURITY):
        ...
        self.beam_uncorrected = ((self.s1 - self.s3)*self.fs)/(self.zbeam * 1.61e-19 * 1000)
        ...
        self.offset_es = self.run_time * self.current_offset
        self.offset_ps = self.offset_es / self.zbeam 

        #self.no_beam = (self.bic_corrected*self.fs)/(self.zbeam*1.61e-19*1000)
        self.no_beam = self.beam_uncorrected * self.current_gradient + self.offset_ps 
        ...
        print(self.beam_uncorrected)
        print(self.fs)
        print(self.efficiency)
        print(self.no_beam)
        print(self.offset_ps)
        print('\n')
        print(self.run_time)
